...
(function() {
  // 밑줄 플러그인 (MDN Selection API 기반 개선)
            ...
            setTimeout(function() {
              try {
                ...
                setTimeout(function() {
                  ...
                  console.log('Underline 서식 적용 완료');
                }, 10);
              } catch (innerError) {
                console.error('Underline 서식 명령 실행 오류:', innerError);
                buttonElement.removeAttribute('data-processing');
              }
            }, 20); // 명령 실행 전 지연
          }
        } catch (outerError) {
          console.error('Underline 서식 적용 중 오류:', outerError);
          buttonElement.removeAttribute('data-processing');
        }
      }, 50); // 주요 실행 지연
    }
