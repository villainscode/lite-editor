...
- [ ] 윗첨자, 아랫첨자 
- [ ] 단축키 지원 
- [ ] 이미지 크롭 (인앱), 정교한 정렬과 리사이징 (width, height)
- [ ] 스마트 링크 (OG 기반)
- [ ] 페이지 긁어오기
- [ ] 자동 동기화 / 클라우드 백업
- [ ] 수식 편집기 LaTax 수식 랜더링 
- [ ] 차트 (바 차트, 원형 차트, 꺽은선 그래프)
- [ ] 인용, 각주 
- [ ] 다크, 라이트 테마 
- [ ] 캘린더 
- [ ] 템플릿 
...
[x] 들여쓰기/내어쓰기 (탭, 시프트+탭) 단축키가 잘 안될때가 있어요.
[x] 컨텐츠의 가장 아래가 코드블록인 경우 그 아래에 일반 텍스트를 넣는 방법이 있으면 좋을 것 같습니당
[ ] clear format이 컨텐츠 구성에 따라서 컨텐츠가 사라지는 경우가 있네용
[ ] 코드블럭 안에서 가로선을 삽입하면 에러가 나유
[ ] 체크박스와 리스트들을 어케하다보면 여러 줄이 한 줄이 되어요
[ ] 넘버링, 불릿, 체크박스 한번 더 클릭하면 원복 되어야 함
[ ] 이미지 클립보드에 있는거 입력 가능하게 
[ ] 현재 bullet list에서 엔터 후 탭키를 두번 누르면 bullet style이 사라지는 버그가 있음 (마이너)
