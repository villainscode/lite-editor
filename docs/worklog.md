...
[x] clear format이 컨텐츠 구성에 따라서 컨텐츠가 사라지는 경우가 있네용
[x] 코드블럭 안에서 가로선을 삽입하면 에러가 나유
[x] 체크박스와 리스트들을 어케하다보면 여러 줄이 한 줄이 되어요
[x] 넘버링, 불릿, 체크박스 한번 더 클릭하면 원복 되어야 함
[x] 이미지 클립보드에 있는거 입력 가능하게 
[x] 현재 bullet list에서 엔터 후 탭키를 두번 누르면 bullet style이 사라지는 버그가 있음 (마이너)
[x] b, u, i, s 단축키 윈도우, 맥 전용 기본키로 다시 설정해야 함 (충돌이 나는 상황이 있을수 있음 - 마이너)
